...
class Load(Asset):
    ...
    def parameters(self, model):
        ...
        self._profile_slice = self._as_sliced_array(
            self.profile,
            model,
            default_value=0.0,
        )

        # Every Load needs its hourly profile.
        ...
        # Keep one scalar zero capex parameter for fixed assets so the
        # capacity(), energy(), power(), and objective() interfaces remain
        # identical for fixed and expandable loads.
        if not self.extensible:
            ...
            setattr(
                model,
                handle,
                pyomo.Param(initialize=0.0),
            )

        # IMPORTANT SIZE REDUCTION:
        # Non-shiftable loads, solar, and wind no longer receive an indexed
        # shift[t] = 0 parameter. Their energy and power methods directly use
        # profile[t] * capacity.
        #
        # Also create shift_cost[t] only when a shiftable formulation actually
        # uses a variable shift cost in the objective.
        needs_shift_cost = (
            self.shiftable
            and (
                self.shift_formulation == "legacy_window"
                or (
                    self.shift_formulation == "cumulative"
                    and self.shift_cost_mode == "charge"
                )
            )
        )
        ...
        if needs_shift_cost:
            if np.isscalar(self.shift_cost):
                shift_cost_profile = np.full(
                    len(self._steps),
                    float(self.shift_cost),
                    dtype=float,
                )
            else:
                shift_cost_profile = self._as_sliced_array(
                    self.shift_cost,
                    model,
                )
        ...
        if self.shiftable and self.shift_formulation == "cumulative":
            ...
            setattr(
                ...
                pyomo.Param(
                    ...
                    initialize=self._param_dict(
                        model.steps,
                        self._shift_power_capacity_slice,
                    ),
                ),
            )
    ...
    def energy(self, model, step=None):
        ...
        if self.shiftable:
            ...
            return (
                profile[step] * capacity
                + shift[step]
            ) * model.time_step

        # Fixed load, solar, and wind use no hourly zero-shift component.
        if step is None:
            return pyomo.quicksum(
                profile[t] * capacity * model.time_step
                for t in model.steps
            )

        return profile[step] * capacity * model.time_step
    ...
    def solution(self, model):
        ...
        for handle in self.handles:
            ...
            if comp.is_indexed():
                solution[handle.split("::")[1]] = self._indexed_component_values(
                    comp,
                    steps,
                )
            else:
                solution[handle.split("::")[1]] = [
                    self._scalar_component_value(comp)
                ]

        profile_comp = getattr(model, f"{self.handle}::profile")
        capex_comp = getattr(model, f"{self.handle}::capex")

        profile = self._indexed_component_values(
            profile_comp,
            steps,
        )
        ...
        if self.shiftable:
            ...
            shift = self._indexed_component_values(
                shift_comp,
                steps,
            )
        else:
            # Preserve the previous output schema without constructing a
            # Pyomo shift[t] parameter for every fixed load/renewable asset.
            shift = [0.0] * len(steps)
            solution["shift"] = shift

        # Preserve the previous output schema even when no Pyomo shift-cost
        # parameter was needed for this asset.
        if "shift_cost" not in solution:
            solution["shift_cost"] = [0.0] * len(steps)
        ...
        if self.shiftable and self.shift_formulation == "cumulative":
            ...
            cumulative_comp = getattr(
                model,
                f"{self.handle}::cumulative_flex_charge",
            )
