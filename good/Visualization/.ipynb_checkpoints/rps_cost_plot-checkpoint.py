...
def plot_capex_delta_regions_by_rps_2x3(
    output_root,
    regions=(
        "CAISO",
        "ERCOT",
    ...
    # Font size inside the stacked bars.
    # When None, it is derived from font_size.
    bar_label_fontsize=None,

    # Optional resource-specific bar-label sizes.
    # Example:
    # {
    #     "Solar": 10,
    #     "Wind": 10,
    #     "Battery": 8,
    # }
    bar_label_fontsize_by_resource=None,
    ...
    # Optional exact y-axis limits by region.
    # Example:
    # {
    #     "CA": (-12000, 6000),
    #     "NY": (-7000, 2000),
    ...
    # Dynamic national layout.
    ncols=3,
    panel_width=6.0,
    panel_height=5.2,
    figsize=None,

    # Optional capacity-data cache and concurrent loading.
    cache_path=None,
    refresh_cache=False,
    region_read_workers=15,
):
    """
    Plot capacity expansion change relative to Base.

    Uses three columns and determines the row count from the number of
    requested regions plus one lower-right legend cell.

    Data logic remains capacity-specific:
        y-axis = Change in added capacity (MW)
        resources = Solar, Wind, Battery
    ...
    symmetric_y_axis:
        If True, y-axis is symmetric around zero.
        Recommended for positive/negative capacity changes.
    ...
    # Font sizes
    # =====================================================
    if bar_label_fontsize is None:
        bar_label_fontsize = max(
            6,
            font_size - 1.5,
        )
    ...
    if bar_label_fontsize_by_resource is None:
        bar_label_fontsize_by_resource = {}
    else:
        bar_label_fontsize_by_resource = {
            str(resource): float(size)
            for resource, size
            in bar_label_fontsize_by_resource.items()
        }
    x_tick_fontsize = max(6, font_size - 2)
    rps_label_fontsize = max(6, font_size - 2)
    scenario_label_fontsize = max(6, font_size - 1)
    ...
    # =====================================================
    # Normalize exact y-axis ranges
    # =====================================================
    if y_limits_by_region is None:
        y_limits_by_region = {}
    else:
        normalized_y_limits = {}
    
        for region_key, limits in y_limits_by_region.items():
            region_code = normalize_region(
                region_key
            )
            ...
            if (
                ...
                raise ValueError(
                    "Each y_limits_by_region value must be "
                    "a two-item tuple or list:\n"
                    "    (minimum, maximum)\n"
                    f"Invalid value for {region_key}: {limits}"
                )
            ...
            if lower_limit >= upper_limit:
                raise ValueError(
                    "The lower y-axis limit must be smaller "
    ...
    # =====================================================
    # Load data strictly by region-specific RPS
    # =====================================================
    cache_metadata = {
        ...
        "baseline_map": dict(baseline_map),
        "program_map": dict(program_map),
        "rps_order_by_region": {
            region: tuple(rps_order_by_region[region])
            for region in region_codes
        },
    }
    ...
    if cache_path is not None:
        ...
        if cache_path.exists() and not refresh_cache:
            ...
            if cached.get("metadata") == cache_metadata:
                ...
                print(f"Loaded capacity cache: {cache_path}")
            else:
                print(
                    "Capacity-cache settings changed. "
                    "Rebuilding the cache."
                )

    def load_one_region(region):
        return collect_capex_delta_for_region_group(
            output_root=output_root,
            region=region,
            year=year,
            adoption=adoption,
    ...
    if not loaded_from_cache:
        worker_count = max(
            1,
            min(int(region_read_workers), len(region_codes)),
        )
        ...
        if not df_list:
            raise ValueError("No capacity data was loaded.")
        ...
        if cache_path is not None:
            ...
            print(f"Saved capacity cache: {cache_path}")
    ...
    if df.empty:
        raise ValueError(
            "Capacity dataframe is empty after applying region-specific RPS filters."
        )
    ...
    for region in region_codes:
        ...
        for part in selected_participation:
            sub = sub_region[
                (sub_region["group"] == selected_group)
                & (sub_region["participation"] == part)
            ].copy()
            ...
            for rps in expected_rps:
                ...
                if sub_rps.empty:
                    ...
                    diagnostics.append({
                        ...
                        "rps": rps,
                        "status": "present",
                        "total_abs_delta_mw": sub_rps["delta_mw"].abs().sum(),
                    })

    capacity_rps_diagnostic_df = pd.DataFrame(diagnostics)

    if print_rps_diagnostics:
        print("\nCapacity RPS diagnostic:")
        print(capacity_rps_diagnostic_df.to_string(index=False))

    if problems:
        message = (
            "Some requested RPS cases are missing from the capacity plot data:\n"
            + "\n".join(problems)
        )
    ...
    if not present_resources:
        raise ValueError(
            "No Solar/Wind/Battery resources were found in the capacity dataframe."
        )

    # =====================================================
    # Dynamic figure: three columns, rows based on regions
    # plus one lower-right legend cell.
    # =====================================================
    ncols = int(ncols)
    ...
    n_required_cells = len(region_codes) + 1
    nrows = int(np.ceil(n_required_cells / ncols))

    if figsize is None:
        figsize = (
            float(panel_width) * ncols,
            float(panel_height) * nrows,
        )
    ...
    # RPS and scenario labels are added after common y-scale is fixed.
    panel_label_records = []
    ...
    for plot_idx, region in enumerate(region_codes):
        ...
        for key in ordered_keys:
            ...
            for seg in label_candidates:
                ...
                resource_label_fontsize = (
                    bar_label_fontsize_by_resource.get(
                        resource,
                        bar_label_fontsize,
                    )
                )
                ...
                ax.text(
                    ...
                    ha="center",
                    va="center",
                    fontsize=resource_label_fontsize,
                    color=get_contrast_text_color(
                        facecolor
                    ),
                    fontweight="normal",
                    clip_on=True,
        ...
        # Save label locations for after y-axis scaling.
        panel_label_records.append({
            "ax": ax,
            "region": region,
            "xticks": xticks,
    ...
    if same_y_scale and plot_axes:
        ...
        for ax in plot_axes:
            ax.set_ylim(common_ymin, common_ymax)
    # =====================================================
    # Apply exact regional y-axis limits
    #
    # These override automatic or common y-axis scaling.
    # =====================================================
    for panel_index, region in enumerate(
        region_codes
    ):
        if region not in y_limits_by_region:
            continue
    ...
    # =====================================================
    # Legend in the lower-right cell
    # =====================================================
    for extra_ax in axes[len(region_codes):]:
        extra_ax.axis("off")
...
def plot_rps_reached_by_wastage_with_adoption_band_regions(
    output_root,
    regions=("CA", "NY", "TX", "FL", "PJM"),
    year=2030,
    SCENARIOS=None,
    ...
    """
    Plot actual curtailment reduction relative to Base.

    Metric
    ------
    curtailment_reduction_gwh =
        Base curtailment - Program curtailment

    Interpretation
    --------------
    Positive:
        V1G or V2G reduces curtailment relative to Base.

    Negative:
        V1G or V2G increases curtailment relative to Base.

    Zero:
        No change relative to Base.

    Main line:
        main_adoption, normally Mid.

    Shaded band:
        range between band_adoptions, normally Slow and Fast.

    The Base case is used for the calculation but is not plotted.
    """
    ...
    if rps_order_by_region is None:
        ...
        rps_order_by_region = {
            normalize_region(key): list(value)
            for key, value in rps_order_by_region.items()
        }
    ...
    if missing_rps_regions:
        raise ValueError(
            "Missing RPS order for these regions:\n"
            + "\n".join(missing_rps_regions)
        )
    ...
    marker_map = {
        participation: marker_sequence[
            index % len(marker_sequence)
        ]
        for index, participation
        in enumerate(selected_participation)
    }
    ...
    band_adoption_labels = [
        str(adoption).capitalize()
        for adoption in band_adoptions
    ]

    if len(band_adoption_labels) != 2:
        raise ValueError(
            "band_adoptions must contain exactly two adoption levels. "
            "Example: ('Slow', 'Fast')."
        )

    low_adoption_label = band_adoption_labels[0]
    high_adoption_label = band_adoption_labels[1]

    # =====================================================
    # Load or reuse cached curtailment data
    # =====================================================
    if cache_path is not None:
        cache_path = Path(cache_path)
    ...
    if use_existing_cache:
        print(
            "Reading cached curtailment data from:\n"
            f"{cache_path}"
        )

        df = pd.read_csv(cache_path)

        if "model_days_in_folder" not in df.columns:
            raise ValueError(
                "The curtailment cache is from the old loader and "
                "does not contain model_days_in_folder.\n"
                "Run once with refresh_cache=True."
            )

        df["model_days_in_folder"] = pd.to_numeric(
            df["model_days_in_folder"],
            errors="coerce",
        )

        df = df[
            df["model_days_in_folder"]
            == int(model_days_per_run)
        ].copy()

        if df.empty:
            raise ValueError(
                f"The cache contains no d{model_days_per_run} results.\n"
                f"Cache: {cache_path}\n"
                "Run once with refresh_cache=True."
            )
        ...
        for adoption in all_adoptions:
            for region in region_codes:
                temp = load_one_region_wastage_json(
                    output_root=output_root,
                    region=region,
                    year=year,
                    SCENARIOS=SCENARIOS,
        ...
        if not all_data:
            raise ValueError(
                "No curtailment data were loaded."
            )
        ...
        if cache_path is not None:
            ...
            print(
                "Saved compact curtailment cache to:\n"
                f"{cache_path}"
            )
    ...
    if not keep_list:
        raise ValueError(
            "No region data remained after RPS filtering."
        )
    ...
    if df.empty:
        raise ValueError(
            "No data remained after the final filters."
        )

    # Ensure one result per plotted case.
    df = collapse_wastage_rows_for_plot(
        df,
        verbose=True,
    )

    # =====================================================
    # Build matching Base table
    # =====================================================
    base_key_cols = [
        "region",
        "adoption",
    ...
    if base_df.empty:
        raise ValueError(
            "No Base-only curtailment rows were found."
        )

    base_df = base_df.rename(
        columns={
            "wastage_gwh":
                "baseline_wastage_gwh",
            "scenario_label":
                "baseline_scenario_label",
            "file_path":
                "baseline_file_path",
    ...
    duplicate_base = base_df[
        base_df.duplicated(
            subset=base_key_cols,
            keep=False,
        )
    ].copy()
    ...
    if not duplicate_base.empty:
        raise ValueError(
            "More than one Base result was found for the "
            "same region, adoption, battery capex, and RPS.\n\n"
            + duplicate_base.sort_values(
                base_key_cols
            ).to_string(index=False)
        )
    ...
    if program_df.empty:
        raise ValueError(
            "No V1G or V2G curtailment rows were found."
        )

    # =====================================================
    # Match every program result to its Base result
    # =====================================================
    df_reduction = program_df.merge(
        base_df,
        on=base_key_cols,
        how="left",
    ...
    if not missing_base.empty:
        ...
        raise ValueError(
            "Some program curtailment results could not be "
            "matched to their Base result.\n\n"
            + missing_base[
                missing_columns
            ]
            .drop_duplicates()
    ...
    #
    # Positive = less curtailment than Base
    # Negative = more curtailment than Base
    # =====================================================
    df_reduction[
        "curtailment_reduction_gwh"
    ] = (
        ...
        - df_reduction["wastage_gwh"]
    )

    # =====================================================
    # Validate requested plot coverage
    # =====================================================
    coverage_problems = []
                ...
                for participation in selected_participation:
                    ...
                    if missing_rps:
                        coverage_problems.append(
                            f"{region}, {adoption}, {group}, "
                            f"{participation}: missing RPS "
                            f"{missing_rps}; available = "
                            f"{available_rps}"
                        )

    if coverage_problems:
        print(
            "\nWARNING: Some plotted combinations are missing:"
        )
    ...
    # Plot each region
    # =====================================================
    for plot_idx, region in enumerate(
        region_codes
    ):
        ax = axes[plot_idx]
        ax.set_facecolor("white")

        region_rps_order = list(
            rps_order_by_region[region]
        )

        rps_labels = get_rps_axis_labels(
            region,
            region_rps_order,
        )

        x_positions = np.arange(
            len(region_rps_order)
        )
        ...
        sub_region = df_reduction[
            df_reduction["region"] == region
        ].copy()
        ...
        # Zero means no curtailment change from Base.
        ax.axhline(
            0,
            color="black",
            linestyle="--",
        ...
        for group in group_order:
            ...
            for participation in selected_participation:
                ...
                low_df = low_df.rename(
                    columns={
                        "curtailment_reduction_gwh":
                            "low_adoption_reduction_gwh"
                    }
                )

                # -----------------------------------------
                # High-adoption side of band
                # -----------------------------------------
                high_df = sub_series[
                    sub_series["adoption"]
                    == high_adoption_label
                ...
                high_df = high_df.rename(
                    columns={
                        "curtailment_reduction_gwh":
                            "high_adoption_reduction_gwh"
                    }
                )

                # -----------------------------------------
                # Slow-to-Fast adoption range in GWh
                # -----------------------------------------
                band_df = low_df.merge(
                    high_df,
                    on="rps_plot_value",
                    how="inner",
                    validate="one_to_one",
                ...
                # -----------------------------------------
                # Mid-adoption GWh-reduction line
                # -----------------------------------------
    ...
    interpretation_handles = [
        Patch(
            ...
            alpha=0.15,
            label=(
                f"{low_adoption_label} to "
                f"{high_adoption_label} "
                "adoption range"
            ),
        ),
        Line2D(
    ...
    # =====================================================
    fig.suptitle(
        "Curtailment reduction relative to Base "
        "across RPS targets\n"
        f"Lines = {main_adoption_label} adoption; "
        f"shaded range = {low_adoption_label} to "
        f"{high_adoption_label} adoption; "
        f"battery capex = ${batt_capex}/kWh",
        fontsize=16,
        y=0.98,
        color="black",
...
def plot_generation_delta_regions_by_rps_2x3_enhanced(
    output_root,
    regions=(
        "CAISO", "ERCOT", "FRCC",
        "ISO-NE", "MAPP", "MISO",
    ...
    # {"CA": 100, "NY": 50}
    #
    # or resource-specific thresholds:
    #
    # {
    #     "CA": {
    #         "default": 80,
    ...
    # Threshold values can remain in GWh even though
    # the graph is plotted in TWh.
    label_threshold_unit="GWh",

    max_labels_per_bar_by_region=None,

    strict_rps_check=True,
    print_rps_diagnostics=False,
    ...
    # -------------------------------------------------
    # Resource and color controls
    # -------------------------------------------------
    resource_color_map=None,
    resource_order=None,
    ...
    """
    Plot generation changes relative to Base in a dynamic three-column figure.

    The number of rows is calculated from the number of requested regions.
    The final bottom-right cell is always reserved for the legend.

    Units
    -----
    The y-axis and plotted values are in TWh.

    Label thresholds
    ----------------
    By default, label thresholds are interpreted in GWh so that
    existing dictionaries such as Solar=120 and Wind=80 do not
    need to be divided by 1000.

    Set:

        label_threshold_unit="TWh"

    to define thresholds directly in TWh.

    Cache behavior
    --------------
    cache_path missing:
        Read model outputs and create the JSON cache.

    cache_path exists and refresh_cache=False:
        Read only the JSON cache.

    refresh_cache=True:
        Read model outputs again and overwrite the cache.

    Formatting changes do not require refreshing the cache.
    """
    ...
    relative_rps_regions = {
        "PJM", "MAPP", "MISO", "ISO-NE", "NWPP",
        "RMRG", "SERC-E", "SERC-N", "SERC-SE",
        "SPP", "SRSG",
    }
    ...
    def _normalize_region(region):
        ...
        if region_text not in region_aliases:
            raise ValueError(
                f"Unknown region: {region}. "
    ...
    def _normalize_required_map(
        ...
        for original_region in original_regions:
            ...
            if original_region in input_map:
                normalized[
                    normalized_region
                ] = input_map[
                    original_region
            ...
            elif normalized_region in input_map:
                normalized[
                    normalized_region
                ] = input_map[
                    normalized_region
    ...
    def _normalize_optional_region_map(
        input_map,
    ):
        if input_map is None:
            return {}
        ...
        return {
            _normalize_region(region): value
            for region, value
            in input_map.items()
        }
    ...
    def _normalize_resource_value_map(
        input_map,
    ):
        if input_map is None:
            return {}
        ...
        return {
            _canonical_resource(resource): value
            for resource, value
            in input_map.items()
        }

    # =====================================================
    # Normalize main model-data inputs
    # =====================================================
    region_codes = [
    ...
    if len(region_codes) != len(set(region_codes)):
        raise ValueError(
            "regions contains duplicate regions after normalization: "
            f"{region_codes}"
        )
    ...
    if not selected_participation:
        raise ValueError(
            "selected_participation cannot be empty."
        )

    baseline_map = _normalize_required_map(
        input_map=(
            baseline_scenario_label_by_region
        ),
    ...
    baseline_map = {
        region: str(label).strip().lower()
        for region, label
        in baseline_map.items()
    }

    program_map = _normalize_required_map(
        input_map=(
            program_scenario_label_by_region
        ),
        original_regions=regions,
    ...
    program_map = {
        region: str(label).strip().lower()
        for region, label
        in program_map.items()
    }

    graph_path_map = _normalize_required_map(
        input_map=graph_json_path_by_region,
        original_regions=regions,
        map_name="graph_json_path_by_region",
    )

    graph_path_map = {
        region: Path(path)
        for region, path
        in graph_path_map.items()
    }

    rps_order_map = _normalize_required_map(
        input_map=rps_order_by_region,
        original_regions=regions,
        map_name="rps_order_by_region",
    )
    ...
    rps_order_map = {
        ...
        for region, values
        in rps_order_map.items()
    }

    panel_title_map = (
        _normalize_optional_region_map(
            panel_title_by_region
        )
    )
    ...
    # =====================================================
    # Normalize regional and resource-specific thresholds
    # =====================================================
    raw_threshold_map = (
        _normalize_optional_region_map(
            label_threshold_by_region
    ...
    for region, region_setting in (
        raw_threshold_map.items()
    ):
        if isinstance(
            region_setting,
            dict,
    ...
    threshold_unit = (
        str(label_threshold_unit)
        .strip()
        .lower()
    )

    if threshold_unit not in {
        "gwh",
        "twh",
    }:
        raise ValueError(
            "label_threshold_unit must be "
            "'GWh' or 'TWh'."
        )

    # =====================================================
    # Validate exact TWh y-axis limits
    # =====================================================
    normalized_y_limits = {}

    for region, limits in y_limits_map.items():
        if (
            not isinstance(
                limits,
                (tuple, list),
            ...
            raise ValueError(
                "Each y_limits_by_region value must "
                "be a two-item tuple/list in TWh:\n"
                "    (minimum_TWh, maximum_TWh)\n"
                f"Bad value for {region}: {limits}"
            )
        ...
        if ymin >= ymax:
            raise ValueError(
                f"Invalid TWh y-axis limits for "
                f"{region}: {limits}. "
                "Require ymin < ymax."
            )
    ...
    if resource_order is None:
        resource_order = (
            default_resource_order.copy()
        )

    else:
        resource_order = [
            _canonical_resource(resource)
            for resource
            in resource_order
        ]
    ...
    # =====================================================
    # Distinct EPA/eGRID-inspired resource palette
    #
    # No two default resources have the same color.
    # =====================================================
    colors = {
        "Solar": "#F4C542",
        "Wind": "#4C78A8",
        ...
        # Requested orange for combined-cycle gas
        "CCNG": "#F28E2B",
    ...
    def _passes_label_threshold(
        ...
        threshold = (
            _get_label_threshold(
                region=region,
                resource=resource,
            )
        )

        if threshold_unit == "gwh":
            return (
                abs(value_gwh)
                >= threshold
            )
    ...
    # =====================================================
    # Maximum labels helper
    # =====================================================
    def _get_max_labels(
        region,
    ):
        value = max_labels_map.get(
            region,
            None,
        )
    ...
    # Cache metadata
    #
    # Plot formatting is intentionally excluded.
    # =====================================================
    expected_cache_metadata = {
        "cache_version": 2,
        "cache_type": (
            "generation_delta_regions_by_rps"
        ),
        "output_root": str(
            Path(output_root)
            .expanduser()
    ...
    if use_existing_cache:
        ...
        if (
            ...
            raise ValueError(
                "The generation cache was created "
                "with different model-data settings.\n\n"
                f"Cache:\n{cache_path}\n\n"
                "Run once with:\n"
                "    refresh_cache=True"
            )
        ...
        print(
            f"Loaded {len(df_gen):,} rows "
            "from cache."
        )

    # =====================================================
    # Collect original data and create/refresh cache
    # =====================================================
    else:
        if cache_path is None:
            print(
                "\nNo generation cache path was "
                "provided. Reading model outputs."
            )

        elif refresh_cache:
            print(
                "\nRefreshing generation-delta "
                "cache from model outputs."
            )

        else:
            print(
                "\nGeneration cache does not "
                "exist yet."
            )
            print(
                "Reading model outputs and "
                "creating the cache."
            )

        # Collect the data directly. Do not call the older 2x3
        # plotting function because that function reserves only
        ...
        for region in region_codes:
            temp = collect_generation_delta_for_region_group(
                ...
                selected_group=selected_group,
                selected_participation=selected_participation,
                rps_order=rps_order_map[region],
                strict_rps_check=strict_rps_check,
            )
        ...
        if not collected_frames:
            raise ValueError(
                "No generation data was loaded for the requested regions."
            )
    ...
    if missing_columns:
        raise ValueError(
            "The generation dataframe/cache "
    ...
    df_gen["resource"] = (
        df_gen["resource"]
        .astype(str)
        .map(_canonical_resource)
    )

    df_gen["rps"] = pd.to_numeric(
        df_gen["rps"],
        errors="coerce",
    )

    df_gen["delta_gwh"] = pd.to_numeric(
        df_gen["delta_gwh"],
        errors="coerce",
    ).fillna(0.0)
    ...
    if not requested_frames:
        raise ValueError(
            "No generation data remain after "
            "applying the requested regions, "
            "group, participation, and RPS."
        )
        ...
        for participation in (
            ...
            if missing_rps:
                coverage_problems.append(
                    ...
                    f"{selected_group}, "
                    f"{participation}: "
                    f"missing RPS {missing_rps}; "
                    f"available = {available_rps}"
                )

            for rps in rps_order_map[
                region
            ]:
                sub_rps = sub_case[
                    sub_case["rps"]
    ...
    extra_resources = sorted(
        resource
        for resource in resources_in_data
        if resource not in resource_order
    )

    final_resource_order = (
        resource_order
        + extra_resources
    )

    # =====================================================
    # Assign unique colors to unknown resources
    # =====================================================
    used_rgba = {
        tuple(
            round(component, 6)
    ...
    for resource in final_resource_order:
        ...
        for candidate in fallback_candidates:
            candidate_rgba = tuple(
                round(component, 6)
                for component
                in to_rgba(candidate)
            )

            if candidate_rgba not in used_rgba:
        ...
        if selected_color is None:
            raise ValueError(
                "Not enough distinct colors were "
    ...
    for resource in final_resource_order:
        ...
        if rgba in color_owner:
            raise ValueError(
                "Duplicate resource color detected:\n"
                f"  {color_owner[rgba]} and "
                f"{resource}\n"
                f"  color = {colors[resource]}\n\n"
                "Assign a different value through "
                "resource_color_map."
            )
    ...
    # =====================================================
    # Create dynamic three-column figure. The last cell is the legend.
    # =====================================================
    ncols = 3
    nrows = int(
        np.ceil(
    ...
    legend_axis_index = total_axes - 1
    ...
    for panel_index, region in enumerate(
        ...
        # Bar positions
        # ---------------------------------------------
        for rps_index, rps_value in enumerate(
            rps_values
        ):
            group_start = (
                rps_index
        ...
        # ---------------------------------------------
        # Stacked generation-change bars
        # ---------------------------------------------
        for rps_value in rps_values:
            for participation in (
                ...
                # -------------------------------------
                # Select labels using regional and
                # resource-specific thresholds
                # -------------------------------------
                label_candidates = []

                for segment in segment_records:
                    if _passes_label_threshold(
                        region=region,
                        resource=segment[
                            "resource"
                        ],
                ...
                # -------------------------------------
                # Draw resource names inside bars
                # -------------------------------------
                for segment in (
                    label_candidates
                ):
        ...
        # ---------------------------------------------
        # RPS labels above groups
        # ---------------------------------------------
        for (
            group_center,
            rps_value,
        ) in rps_label_positions:
    ...
    # =====================================================
    # Disable unused regional panels
    # =====================================================
    ...
    if same_y_scale:
        ...
        for ax in regional_axes:
            ...
            if symmetric_y_axis:
                ...
                ax.set_ylim(
                    current_ymin - padding,
                    current_ymax + padding,
                )

    # =====================================================
    # Exact regional TWh limits override automatic limits
    # =====================================================
    for region, limits in (
        y_limits_map.items()
    ):
        if region in region_axis_map:
            region_axis_map[
                region
            ].set_ylim(
                limits
    ...
    # =====================================================
    # Legend in the bottom-right cell
    # =====================================================
    legend_axis = axes_flat[
        legend_axis_index
    ]
    ...
    ordered_legend_resources = [
        resource
        for resource
        in final_resource_order
        if resource
        in resources_in_legend
    ]
